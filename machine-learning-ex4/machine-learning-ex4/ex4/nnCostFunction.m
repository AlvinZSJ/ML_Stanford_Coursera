...
J_individual = log(output) .* (-y_label) - log(1 - output) .* (1 - y_label);
% fprintf(['J_individual %f \n'], J_individual);

Theta1_square = Theta1(:,2:end) .^ 2;
Theta2_square = Theta2(:,2:end) .^ 2;
J = 1 / m * sum(J_individual(:)) + lambda / (2 * m) * (sum(Theta1_square(:)) + sum(Theta2_square(:)));

delta_3 = zeros(1, num_labels);
num_layers = 3;

for i=1:m
  delta_3 = output(i,:) - y_label(i,:);
  delta_2 = Theta2' * delta_3' .* [1; sigmoidGradient(z_2(i,:)')];
  delta_2 = delta_2(2:end);
  Theta1_grad += delta_2 * a_1(i,:); 
  Theta2_grad += delta_3' * a_2(i,:);
endfor
...
%gradient_layer = zeros(num_layers, )
